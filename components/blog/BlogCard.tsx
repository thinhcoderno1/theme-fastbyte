...
export function BlogCard({ post, featured = false }: { post: WordPressPost; featured?: boolean }) {
  ...
  return (
    <article className={`group overflow-hidden rounded-xl border border-line bg-white shadow-sm ${featured ? 'md:grid md:grid-cols-2' : ''}`}>
      <Link href={`/blog/${post.slug}/`} className={`block overflow-hidden bg-surface-blue ${featured ? 'min-h-64' : 'aspect-[16/9]'}`}>
        <BlogImage
          post={post}
          priority={featured}
      ...
      <div className={`flex flex-col ${featured ? 'p-7 lg:p-9' : 'p-5'}`}>
        ...
        <Link href={`/blog/${post.slug}/`} className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-brand-700 hover:text-brand-900">
