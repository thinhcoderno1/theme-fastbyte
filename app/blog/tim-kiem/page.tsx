...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  return (
    <>
      <section className="border-b border-line bg-surface-blue py-12">
        <Container>
          <h1>Tìm kiếm bài viết</h1>
          <BlogSearchForm defaultValue={query} className="mt-6" />
        </Container>
      </section>
      <Container className="py-12 md:py-16">
        {query && <p className="mb-6 text-sm text-ink-600">Kết quả cho: <strong className="text-ink-900">{query}</strong></p>}
        {content}
      </Container>
    </>
  );
